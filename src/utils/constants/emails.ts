...
export const registrationVerificationEmail = (
  ...
  background-repeat: repeat; line-height: 1.6; margin: 0; padding: 20px; overflow: hidden; position: relative;  margin: auto;">
  <div style="color: #ffffff; width:80%">
    <h2 style="color: #c9a747; font-weight: 600; font-size: 2rem; letter-spacing:3px;">Registration Verified for ${eventName} in Regalia 2024</h2>
    <h4 style="font-size:1.4rem">
      Dear ${participants && participants?.length > 1 ? "Team" : ""} ${teamName},
    </h4>
    <p  style="font-size:1.2rem;color:#ffffff">
    Congratulations ! Your registration for Event <strong  style="letter-spacing:2px; color: #c9a747">${eventName}</strong> has been successfully verified . Your enthusiasm for participation is truly appreciated, and we're looking forward to an exciting event with your energetic performance ahead ! All The Best !

    </p>
    <h3 style="color: #ffffff; font-weight: 500; font-size:1.4rem">Registration Details :</h3>
    <ul style="color: #ffffff; font-size:1.2rem">
    ${participants && participants.length > 1 ? `<li><strong> Team Name:</strong> ${teamName}</li>` : ""}
      <li><strong>${participants && participants?.length > 1 ? "Team Leader" : ""} Name:</strong> ${team.name}</li>
      <li><strong>${participants && participants?.length > 1 ? "Team Leader" : ""} Phone:</strong> <a href="tel: ${clearSpaces(team.phone).trim()}" style="color:#008000 ">${clearSpaces(team.phone).trim()}</a></li>
      <li style=""><strong>Email:</strong> <a href=${`mailto: ${team.email}`} style="color:#008000 ; font-size:1.5rem; letter-spacing:1px">${team.email}</a></li>
    </ul>
    
    <div style="margin-bottom:20px; margin-top: 20px;">
    <a href="${whatsAppLink}" style="display: inline-block; background-color: #25D366; color: #ffffff; font-size: 1.2rem; font-weight: bold; text-decoration: none; padding: 15px 25px; border-radius: 5px; border: none; cursor: pointer; transition: background-color 0.3s ease;">
      Join WhatsApp Group
    </a>
  </div>
      ...
      `<div style="">
        ...
        .map(
          (participant: any, index: number) =>
            `<tr key=${index}>
                  <td style=" padding:5px 20px; text-align: center;">${participant.name}</td>
                  <td style=" padding: 5px 20px; text-align: center; "><a href="tel: ${participant.phone}" style="color:#008000">${participant.phone}</a></td>
              </tr>`,
        )
        .join("")}
          </tbody>
   ...
   <div style="width:80%">
   <strong style="font-size:1.2rem ; color:#ffffff">
   <span style="color:#FF0000 ; font-size:1.4rem;  letter-spacing:2px">Join the WhatsApp group through the above attached link for further updates regarding the event schedules and instructions . For any kind of queries, feel free to ask in the group .</span> Feel free to reach out to us at <a href="mailto: regalia.rcciit.official@gmail.com" style="color:#c9a747; letter-spacing:3px ; font-size:1.5rem; font-weight:600">regalia.rcciit.official@gmail.com</a>.
 </strong>
 <br>
 <p style="font-size:1.4rem; color:#ffffff">
   Thank you once again for choosing to be a part of the <strong  style="letter-spacing:2px; color: #c9a747">${eventName}</strong>. We wish you the best of luck, and we can't wait to see you showcase your talents!
